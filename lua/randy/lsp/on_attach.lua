...
local function attach(client, bufnr)
    ...
    vim.schedule(function()
        vim.notify("Attached LSP: "..client.name.." to buf #"..bufnr)
    end)
    ---------------------------------------------------------------------------
    ...
    -- inside on_attach, before mapping that uses them
    local ok_omni, omni = pcall(require, 'omnisharp_extended')
    ...
    if ok_tb then
        nmap('<leader>ld',  tb.lsp_document_symbols,         '[L]ist [D]ocument Symbols')
        nmap('<leader>lw',  tb.lsp_dynamic_workspace_symbols,'[L]ist [W]orkspace Symbols')
        nmap('<space>u',    tb.lsp_references,               'Search [U]sages')
        nmap('gr',          tb.lsp_references,               '[G]oto [R]eferences')
    end

    nmap('gI',          vim.lsp.buf.implementation,          '[G]oto [I]mplementation')
    nmap('<space>i',    vim.lsp.buf.implementation,          'Goto [I]mplementation')
    nmap('<leader>D',   vim.lsp.buf.type_definition,         'Type [D]efinition')
    nmap('<leader>k',   vim.lsp.buf.hover,                   'Hover Documentation')
    nmap('<leader>lh',  vim.lsp.buf.signature_help,          '[L]ist Signature Documentation [H]elp')
    nmap('<space>G',    vim.lsp.buf.declaration,             '[G]oto Declaration')
    ...
    nmap('<leader>lf', function() vim.lsp.buf.format({ bufnr = bufnr }) end, '[L]SP Format')
end
